...
(function (global) {
  ...
  /**
   * Determine the absolute site root from wherever app.js is served.
   * app.js lives at: <SITE_ROOT>/assets/js/app.js
   * This works correctly on GitHub Pages subdirectory repos
   * (e.g. username.github.io/reponame/) AND on root deployments.
   */
  const BASE = (function () {
    const s = document.querySelector('script[src*="app.js"]');
    if (s && s.src) {
      // Strip "assets/js/app.js" suffix → gives us the site root with trailing /
      return s.src.replace(/assets\/js\/app\.js(\?.*)?$/, '');
    }
    // Fallback: count path segments to walk up to root
  ...
  async function initHomepage() {
    const [articles, categories] = await Promise.all([
      fetchJSON(BASE + 'content/articles.json'),
      fetchJSON(BASE + 'content/categories.json')
    ]);
